    ...
    <script>
        $(document).ready(function(){
            $("#btn_Create").click(function(){
                window.location.href="${ctx}/dicta/create";
            }) ;
            $(".cla_delete").bind("click",function () {
                if (confirm("确定要删除吗？")){
                    var id=$(this).attr("id");
                    window.location.href="${ctx}/dicta/delete/"+id;
                }
            });
            $(".cla_edit").bind("click",function () {
                window.location.href="${ctx}/dicta/update/${dict.id}";
            })
            $("#btn_Delete").bind("click",function () {
                ...
                if(count<1){
                    ...
                    if (confirm("确定要删除所选项吗？")){
                        $("#listFrm").attr({action:"${ctx}/dicta/delete", method:"POST"})
                        $("#listFrm").submit();
                    }
                }
...
<div class="container">
    ...
    <table id="contentTable" class="table table-striped table-hover">
        <thead><tr>
            ...
            <th>管理</th></tr></thead>
        <tbody>
        <c:forEach items="${dicts.content}" var="dicta" varStatus="idxStatus">
            <tr>
                ...
                <td>
                    <a href="${ctx}/dicta/update/${dict.id}">编辑</a>
